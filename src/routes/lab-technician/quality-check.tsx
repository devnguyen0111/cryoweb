...
/**
 * ===========================
 * NOTE TỰ HỌC, NHỮNG CHỖ QUAN TRỌNG
 * ===========================
 *
 * 1) Vì sao view trắng dù response có dữ liệu
 *    GET /api/labsample/{id} trả data dạng:
 *    data: { sperm, oocyte, embryo, patient, ... }
 *    nên field của embryo nằm ở data.embryo.grade, không phải data.grade
 *
 * 2) Patient info trước đó không hiện vì sai path
 *    Thường backend trả: patient.accountInfo.firstName, lastName, gender, birthDate
 *    nên phải lấy đúng: viewData.patient?.accountInfo?.firstName
 *
 * 3) View phải gọi GET detail, không dùng data từ list
 *    List thường thiếu field con, nên bấm View hoặc Update phải gọi getSampleById(id)
 *
 * 4) Update chia 3 nhánh theo sampleType
 *    - Sperm: api.sample.updateSpermSample(id, payload)
 *    - Oocyte: api.sample.updateOocyteSample(id, payload)
 *    - Embryo: api.sample.updateEmbryoSample(id, payload)
 *
 * 5) View và Update tách riêng
 *    View chỉ hiển thị read only, không dùng input để tránh rối và tránh bind sai
 */
...
function QualityCheckPage() {
  ...
  const openView = async (row: any) => {
    ...
    try {
      ...
      setViewData((res as any)?.data ?? null);
    } catch (e) {
  ...
  <div className="grid grid-cols-2 gap-6">
    <ViewField
      label="Patient Code"
      value={viewData.patient?.patientCode ?? "-"}
    />
    ...
    <ViewField
      label="Phone"
      value={viewData.patient?.phoneNumber ?? "-"}
    />

    <ViewField
      label="Email"
      value={viewData.patient?.email ?? "-"}
    />
  </div>
</InfoSection>
